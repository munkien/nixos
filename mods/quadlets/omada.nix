...
in
{
  ...
  systemd.tmpfiles.rules = map (d: "d /persist/services/omada${d} 0755 root root -") [ "" "/db" "/data" "/logs" ];

  # 2. Networking & Firewall
  networking.firewall = {
    allowedTCPPortRanges = [ { from = 29811; to = 29817; } ];
  ...
  # 3. Containers
  virtualisation.quadlet.containers.omada-db = lib.recursiveUpdate common {
    containerConfig = {
      ...
      volumes = [
        "/persist/services/omada/db:/data/db:rw,Z,U"
      ];
    };
  };
