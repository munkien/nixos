{
  ...
  persistBase =
    ...
    then "${config.my.impermanence.persistPath}/services/omada"
    else "/var/lib/omada";
in {
  options.my.services.omada.enable = lib.mkEnableOption "Omada Controller";
  config = lib.mkIf config.my.services.omada.enable {
